...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  for sub_dir in sub_dirs:
    ...
    file_list = []
    dir_name = os.path.basename(# tf.gfile.Walk() returns sub-directory with trailing '/' when it is in
    ...
    for file_name in file_list:
      ...
      # deciding which set to put an image in, the data set creator has a way
      # of
      # grouping photos that are close variations of each other.  For example
      # this is used in the plant disease data set to group multiple pictures
      # of
      # the same leaf.
...
def prepare_file_system():
  ...
  if tf.gfile.Exists(os.path.join(FLAGS.workspace_dir, FLAGS.summaries_dir)):
    tf.gfile.DeleteRecursively(os.path.join(FLAGS.workspace_dir, FLAGS.summaries_dir))
  tf.gfile.MakeDirs(os.path.join(FLAGS.workspace_dir, FLAGS.summaries_dir))
  if FLAGS.intermediate_store_frequency > 0:
    ensure_dir_exists(os.path.join(FLAGS.workspace_dir, FLAGS.intermediate_output_graphs_dir))
    ...
    if not os.path.exists(_retrain_checkpoint_path):
        os.mkdir(_retrain_checkpoint_path)
...
def main(_):
  ...
  if not os.path.join(FLAGS.workspace_dir, FLAGS.image_dir):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    if do_distort_images:
      ...
      cache_bottlenecks(sess, image_lists, FLAGS.image_dir,
                        os.path.join(FLAGS.workspace_dir, FLAGS.bottleneck_dir), jpeg_data_tensor,
                        decoded_image_tensor, resized_image_tensor,
                        bottleneck_tensor, FLAGS.tfhub_module)
    ...
    train_writer = tf.summary.FileWriter(os.path.join(FLAGS.workspace_dir, FLAGS.summaries_dir) + '/train',
                                         sess.graph)

    validation_writer = tf.summary.FileWriter(os.path.join(FLAGS.workspace_dir, FLAGS.summaries_dir) + '/validation')
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if do_distort_images:
        ...
        (train_bottlenecks,
         train_ground_truth, _) = get_random_cached_bottlenecks(sess, image_lists, FLAGS.train_batch_size, 'training',
             os.path.join(FLAGS.workspace_dir, FLAGS.bottleneck_dir), FLAGS.image_dir, jpeg_data_tensor,
      ...
      if (intermediate_frequency > 0 and (i % intermediate_frequency == 0) and i > 0):
        ...
        train_saver.save(sess, os.path.join(FLAGS.workspace_dir, FLAGS.checkpoint_path))
        intermediate_file_name = (os.path.join(FLAGS.workspace_dir, FLAGS.intermediate_output_graphs_dir) + 'intermediate_' + str(i) + '.pb')
    ...
    # constants.
    tf.logging.info('Save final result to : ' + os.path.join(FLAGS.workspace_dir, FLAGS.output_graph))
    if wants_quantization:
      tf.logging.info('The model is instrumented for quantization with TF-Lite')
    save_graph_to_file(os.path.join(FLAGS.workspace_dir, FLAGS.output_graph), module_spec, class_count)
    with tf.gfile.GFile(os.path.join(FLAGS.workspace_dir, FLAGS.output_labels), 'w') as f:
      f.write('\n'.join(image_lists.keys()) + '\n')

    if os.path.exists(os.path.join(FLAGS.workspace_dir, FLAGS.saved_model_dir)):
      export_model(module_spec, class_count, os.path.join(FLAGS.workspace_dir, FLAGS.saved_model_dir))
